...
class modifier extends database {
    ...
    public function edition() {
        ...
        if(isset($_SESSION['id']))
        {
            ...
            if(isset($_POST['newPrenom']) AND !empty($_POST['newPrenom']) AND $_POST['newPrenom'] != $user['prenom'])
            {
                ...
                header("Location: profilmembre.php?id=".$_SESSION['id']);
            ...
            if(isset($_POST['newMail']) AND !empty($_POST['newMail']) AND $_POST['newMail'] != $user['mail'])
            {
                ...
                $insertprenom = $this->connect()->prepare("UPDATE membres SET mail = ? WHERE id = ?");
                $insertprenom->bindParam(1, $newprenom);
                $insertprenom->bindParam(2, $_SESSION['id']);
                $insertprenom->execute();
                header("Location: profilmembre.php?id=".$_SESSION['id']);
            ...
            if(isset($_POST['newMdp']) AND !empty($_POST['newMdp']) AND isset($_POST['newMdp2']) AND !empty($_POST['newMdp2']))
            {
                ...
                if($mdp == $mdp2)
                {
                    $insertmdp = $this->connect()->prepare("UPDATE membres SET motdepasse = ? WHERE id = ?");
